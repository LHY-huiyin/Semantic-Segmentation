...
class Trainer(object):
    def __init__(self, args):
        ...
        # band = 384
        # Define network
        model = MACUNet(band_num=3, class_num=self.nclass)  # 输入通道：一开始是三通道的图片
        # model = MACUNet(backbone=args.backbone, band_num=384, class_num=8)
        #
        # # 构建一个优化参数列表
    ...
    def validation(self, epoch):
        ...
        # 保存文件
        with codecs.open('实验记录resnet50_MACU_Net.txt', 'a', 'utf-8') as f:
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=18,  # 2,4,8,12,14

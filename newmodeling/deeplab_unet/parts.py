...
class DecoderUnetSCSE(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(in_channels, middle_channels, kernel_size=3, padding=1),  # [4,512,24,24]
    ...
    def forward(self, *args):
        x = torch.cat(args, 1)  # 先从concatenate  [4,256,24,24]  [4,1024,24,24]
        return self.block(x)  # 再进行卷积，注意力机制，反卷积

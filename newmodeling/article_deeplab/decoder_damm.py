...
class Decoder(nn.Module):
    def __init__(self, num_classes, backbone, BatchNorm, bilinear=True):
        ...
        self.msapp_conv = nn.Sequential(nn.Conv2d(low_level_inplanes, 256, kernel_size=1, stride=1, bias=False),
                                        # low_level_inplanes = 256 -> [batch, 48, w, h])
                                        BatchNorm(256),
                                        nn.ReLU())

        self.PAM_3 = PAM_Module(4*low_level_inplanes)
        self.PAM_4 = PAM_Module(8*low_level_inplanes)

        # MASPP+位置注意力模块
        # if bilinear, use the normal convolutions to reduce the number of channels
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.conv = DoubleConv(13*low_level_inplanes, low_level_inplanes, 13*low_level_inplanes // 2)
        else:
            self.up = nn.ConvTranspose2d(13*low_level_inplanes, 13*low_level_inplanes // 2, kernel_size=2, stride=2)
            self.conv = DoubleConv(13*low_level_inplanes, low_level_inplanes)
    ...
    def forward(self, maspp, x4, x3, x2, x1):  # resnet:maspp:[4,256,32,32] x4:[4,2048,32,32] x3:[4,1024,32,32] x2:[4,512,64,64] x1:[4,256,128,128]
        ...
        maspp1 = self.msapp_conv(maspp)
        # 将x3,x4进行位置注意力机制
        x3 = self.PAM_3(x3)
        x4 = self.PAM_4(x4)
        # 将x3,x4,MASPP聚合，并4倍上采样  ->1/4
        m = torch.cat((maspp1, x4, x3), dim=1)  # [4,3328,32,32]
        m = self.conv(m)  # [4,256,32,32]
        m = F.interpolate(m, size=x1.size()[2:], mode='bilinear', align_corners=True)  # 4倍上采样  [4,256,128,128]
        # 将x2进行2倍上采样
        m1 = F.interpolate(x2, size=x1.size()[2:], mode='bilinear', align_corners=True)  # 2倍上采样 [18,256,96,96]
        # 将x1进行1*1卷积
        m2 = self.msapp_conv(x1)  # [18,256,96,96]
        # 将x1,x2,x3,x4进行组合
        m3 = torch.cat((m, m1, m2), dim=1)  # [18,1024,96,96]
        # 进行3*3卷积

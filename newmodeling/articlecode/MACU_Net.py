...
class MACUNet(nn.Module):
    def __init__(self, band_num, class_num):
        ...
        self.band_num = band_num  # 输入通道数
        self.class_num = class_num
        self.name = 'MACUNet'

        # channels = [32, 64, 128, 256, 512]
        channels = [16, 32, 64, 128, 256, 512]   # 通道数
    ...
    def forward(self, x):
        conv1 = self.conv1(x)  # 经过两个ACB结构（三个卷积构成：3*3卷积、1*3卷积、3*1卷积）
        conv12 = self.conv12(conv1)  # 经过一个最大池化+一个ACB结构
        conv13 = self.conv13(conv12)
        conv14 = self.conv14(conv13)

        conv2 = self.conv2(conv1)  # 经过一个最大池化+两个ACB结构
        conv23 = self.conv23(conv2)  # 经过一个最大池化+一个ACB结构
        conv24 = self.conv24(conv23)

        conv3 = self.conv3(conv2)  # 经过一个最大池化+三个ACB结构
        conv34 = self.conv34(conv3)

        conv4 = self.conv4(conv3)  # 经过一个最大池化+三个ACB结构

        conv5 = self.conv5(conv4)  # 经过一个最大池化+三个ACB结构

        deconv4 = self.deconv4(conv5)  # 反卷积
        deconv43 = self.deconv43(deconv4)
        deconv42 = self.deconv42(deconv43)
        deconv41 = self.deconv41(deconv42)

        conv6 = torch.cat((deconv4, conv4, conv34, conv24, conv14), 1)  # CAB4
        conv6 = self.skblock4(conv6)  # 通道注意力
        conv6 = self.conv6(conv6)  # 两个ACB结构

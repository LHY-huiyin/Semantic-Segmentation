...
class Trainer(object):
    ...
    def validation(self, epoch):
        ...
        # 保存文件
        with codecs.open('实验记录resnet101_pointflow.txt', 'a', 'utf-8') as f:
        ...
        if new_pred >= 0.52:  # MIOU
            for i, sample in enumerate(tbar):
                ...
                for j in range(4):
                    ...
                    # plt.axis('off')
                    "opencv保存彩色图片"
                    image_name = "./RESULT/PointFlow/" + str(i) + '_' + str(j) + '.jpg'
                    segmap = segmap[:, :, ::-1] * 255
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=12,  # 2,4,8,12,14，16，18

...
class SegmentationLosses(object):
    ...
    def MyLoss(self, logit, target, supervisor, gamma=2, alpha=0.5):
        ...
        f3 = self.FocalLoss(F.upsample_bilinear(supervisor[1], scale_factor=16), target)
        f4 = self.FocalLoss(F.upsample_bilinear(supervisor[2], scale_factor=8), target)

        return 2 * f1 + f2 + f3 + f4
        # pass
...
class ECELoss(nn.Module):
    def __init__(self, n_classes=8, alpha=1, radius=1, beta=0.5, ignore_lb=255, mode='ohem', batch_average=True, *args,
                 **kwargs):
